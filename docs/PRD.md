...
| ③ 分句结果 | **分层展示（el-tabs）**：`句子层`（每句卡片：序号、字数、tier 标签、置信度百分比、原文；「生成提示词」「生图对比」「复制原文」按钮）+ `场景与字幕层`（每场景卡片：场景文本、时长、字幕块表格——序号/字幕文本/起止时间） | 分句完成后展示；句子层可单独生成提示词/图片；场景与字幕层只读展示分句引擎全部层级结果；>30 句提示生图成本 |
