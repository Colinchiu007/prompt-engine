...
如果优化结果是英文，结果下方会显示「🇨🇳 中文翻译」折叠区。点击展开后可看到基于关键词替换的简易中文翻译，并可一键复制。

**注意**：
- 翻译仅供展示理解，**不应用于图片生成**
- 复制到图片生成平台的应是英文原文
- 翻译基于 200+ 关键词中英对照表，非机器翻译，句式不一定通顺
- 若需完美翻译，可使用 LLM 翻译
...
A: 请确认请求中携带了完整的 `llm` 对象（provider、model、api_key）。缺少调用方绑定时接口会返回 422；引擎不会从自身配置文件兜底。
