...
全部 181 个测试通过，使用 mock 隔离，无需真实 API Key。
...
- **v0.7.0** — 模板驱动优化（YAML 模板）、多模型供应商（Gemini）、Web 看板（Vue 3 + Element Plus）、评估对比模式（`POST /v1/evaluate`）、DSL 模板语法（`{a|b}` / `__wild__` / `{N$$opt}`）
