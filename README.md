...
全部 190 个测试通过，使用 mock 隔离，无需真实 API Key。
...
- **v0.8.0** — Dashboard 资源展示、Workbench 图片预览（14 模型）、Settings 图片模型 API 配置、评估对比模式（`POST /v1/evaluate`）、DSL 模板语法（`{a|b}` / `__wild__` / `{N$$opt}`）
