...
class XfyunProvider(BaseLLMProvider):
    ...
    def chat(self, messages: list) -> tuple:
        ...
        logger.info(
            "Xfyun request: model=%s base_url=%s messages=%d total_chars=%d temp=%.1f max_tokens=%d key=%s",
        ...
        try:
            ...
            logger.info(
                "Xfyun response: model=%s tokens=%d (prompt=%d completion=%d) latency_ms=%d output_len=%d",
                self._model, tokens, prompt_tokens, completion_tokens, elapsed_ms, len(text),
            )
            return text, tokens
        except Exception as e:
            elapsed_ms = int((time.time() - _start) * 1000)
            logger.error("Xfyun error: model=%s latency_ms=%d key=%s error=%s", self._model, elapsed_ms, _key_prefix, str(e)[:300])
            raise
