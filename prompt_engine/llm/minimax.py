...
class MiniMaxProvider(BaseLLMProvider):
    ...
    def chat(self, messages: list) -> tuple:
        _start = time.time()
        logger.info("MiniMax request: model=%s base_url=%s messages=%d", self._model, self._client.base_url, len(messages))
        try:
            ...
            logger.info("MiniMax response: model=%s tokens=%d latency_ms=%d", self._model, tokens, elapsed_ms)
            return text, tokens
        except Exception as e:
            elapsed_ms = int((time.time() - _start) * 1000)
            logger.error("MiniMax error: model=%s latency_ms=%d error=%s", self._model, elapsed_ms, str(e)[:200])
            raise
