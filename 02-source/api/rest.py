...
async def image_preview(request: dict):
    ...
    if model == "MiniMax":
        ...
        try:
            ...
            # 解析 MiniMax 响应（记录完整响应以便调试）
            raw_data = data
            logger.info("MiniMax API response: %s", json.dumps(raw_data, ensure_ascii=False)[:500])
...
def _load_env_file():
    """启动时加载 .env 文件到 os.environ（仅设置尚未存在的变量）。"""
    import os
    if ENV_FILE.exists():
        with open(ENV_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line and not line.startswith("#") and "=" in line:
                    ...
                    k, v = k.strip(), v.strip()
                    if v and v not in ("your_...", "your_k...here"):
...
def _get_current_provider() -> str:
    ...
    try:
        ...
        return cfg.get("llm", {}).get("provider", "minimax")
    except Exception:
        return "minimax"


@app.get("/v1/config/llm-provider")
async def get_llm_provider():
    """返回当前默认的 LLM 供应商。"""
    return {"provider": _get_current_provider()}


@app.post("/v1/config/llm-provider")
async def set_llm_provider(request: dict):
    """设置默认 LLM 供应商（写入 config.yaml，需重启生效）。"""
    provider = request.get("provider", "")
    valid = ["openai_compat", "xfyun", "gemini", "minimax", "deepseek"]
    if provider not in valid:
        raise HTTPException(status_code=400, detail=f"无效供应商: {provider}，可选: {valid}")
    ...
    try:
        ...
        return {"provider": provider, "saved": True, "hint": "已保存，重启后端服务后生效"}
    except Exception as e:
