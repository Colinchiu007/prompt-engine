...
class VideoOptimizeRequest(BaseModel):
    ...
    max_length: int = Field(default=1800, ge=200, le=40000, description="优化结果最大字符数（批量层默认 1800 字符；精修层 creative_level≥7 上限 40000，对齐契约层 videoMaxLengthMax——500-5000 词导演分镜单 ≈22871 字符）")
...
class VideoFeedbackRequest(BaseModel):
    ...
    result_prompt: str = Field(..., min_length=1, max_length=40000, description="引擎输出的优化提示词（refined 层上限 40000，对齐 max_length 边界上浮）")
