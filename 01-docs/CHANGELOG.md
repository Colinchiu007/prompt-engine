...
- **新 API 路由注册失败** — `ENV_FILE = Path(...)` 在模块级别使用 `Path`，但 `from pathlib import Path` 只在函数内部 import，导致 `rest.py` 加载到 600 行崩溃，之后的路由（含新 API Key 端点、StaticFiles 挂载）全部 404
- **StaticFiles 拦截 API 路由** — `app.mount("/", StaticFiles(...), html=True)` 挂在根路径，导致所有 `/v1/...` 请求被静态文件中间件拦截返回 index.html。修复：移到 `/web`

### P1 新增功能

- **DeepSeek LLM Provider** — 新增 `prompt_engine/llm/deepseek.py`，Endpoint `api.deepseek.com`，模型 `deepseek-v4-pro`（OpenAI 兼容）。在 `llm/__init__.py`、`llm/base.py` 注册，`config.yaml` 添加 `deepseek` 区块
- **MiniMax LLM Provider** — 新增 `prompt_engine/llm/minimax.py`，支持 MiniMax-M3 模型作为提示词优化的 LLM 供应商，默认 LLM 切换为 `minimax`
- **API Key 配置 UI** — Settings 页面 LLM 供应商卡片内直接嵌入 API Key 输入（密码框 + 三方密钥存储），不再需要翻到底部图片模型区域
- **保存即时生效** — `os.environ[KEY] = val` 在保存时即时注入，无需重启后端
- **默认供应商持久化** — `GET/POST /v1/config/llm-provider` 端点，前端下拉切换后自动保存到 `config.yaml`
- **供应商 Key 状态标识** — 下拉选项中已配置 API Key 的供应商显示 ✅ 前缀
- **重定向** — `GET /` -> 302 -> `/web`，避免用户访问根路径空白
- **配置安全** — `.env` 加入 `.gitignore`；API Key 写入 `.env` 不返回明文；`config.yaml` 中硬编码的 xfyun key 改为 `${XFYUN_API_KEY}` 环境变量引用

### 变更文件

| 文件 | 说明 |
|------|------|
| `prompt_engine/llm/deepseek.py` | **新增** DeepSeek LLM Provider |
| `prompt_engine/llm/minimax.py` | **新增** MiniMax LLM Provider |
| `prompt_engine/llm/__init__.py` | 注册 `minimax`、`deepseek` 供应商 |
| `prompt_engine/llm/base.py` | `from_config` 添加 `minimax`、`deepseek` 分支 |
| `prompt_engine/api/rest.py` | 新增 `/v1/config/api-key`、`/v1/config/llm-provider` 端点；`from pathlib import Path` 移至模块级别；StaticFiles 挂载 `/` → `/web`；`import os` 模块级别；`import yaml`；API Key 注入 `os.environ` 即时生效 |
| `prompt_engine/web/index.html` | Settings API Key 输入嵌入 LLM 供应商卡片；供应商下拉 ✅ 状态标识；默认供应商自动加载 + 切换保存；`imageModelsData` 导出修复；版本号 v0.21.0；移除 `setTimeout` fallback |
| `prompt_engine/optimizer.py` | `disturb_and_optimize` 修复 |
| `config.yaml` | 新增 `minimax`、`deepseek` 区块；xfyun key 改为 `${XFYUN_API_KEY}` 环境变量 |
| `.gitignore` | 新增 `.env` 忽略 |
| `.env` | 模板文件（各供应商环境变量名说明、占位符） |
